...
- [Week 9-10](#20210427) - [v]
    - 모델 설계 공부 및 모델 설계
  
- [Week 11-12](#20210511) - [v]
    - 모델 정확도 검증
  
- [Week 13-14](#20210601) - [v]
    - 데이터 시각화 및 프레젠테이션 작업
---
### Member
...
PBL1 이후
- 17,18년도 데이터셋 통합
- 많은 결측치는 제거, 이상치 제거, 이외의 데이터들은 보존
- 각자 통합된 데이터셋을 가지고 모델을 돌려보는 시간을 갖기로함.
...
- 선택한 모델을 20만개의 데이터를 가지고 검증할것임.
어떤 metrics를 이용해서, 어떤 방법으로 각각의 모델의 정확도를 보여주고 그 중에서 어떤 모델이 가장 적합하다 라고 신빙성 있게 뒷받침해야함.

주의점
- Test데이터와 Training데이터 각각에 있는 positive sample과 negative sample의 비율이 가능한한 같게 해야한다.
---
### 2021.05.11
> Week 11
...
다중분류 결과 토의
- 정확도가 매우 낮았음
  -> 이중분류로 하되 발표 마지막에 가장 높은 성능을 내었던 모델을 가지고 다중분류를 하기로 결정
